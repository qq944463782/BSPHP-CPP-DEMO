...
// XML 解析：提取 <tag>value</tag> 中的 value | XML parse
...
std::string BsPhp::_send_data(const std::string& api,
    ...
    if (!resp.appsafecode.empty() && resp.appsafecode != appsafecode) {
        return "";  // appsafecode security validation failed
    }
    return raw;
}
...
ApiResponse BsPhp::login_lg(const std::string& user, const std::string& password,
                            const std::string& coode, const std::string& key,
                            const std::string& maxoror) {
    std::map<std::string, std::string> param = {
        {"user", user},
    ...
    if (result.empty()) return ApiResponse();
    ApiResponse resp = parse_response(result);
    // Keep session valid for both normal login and "expired" login (Mac treats 9908 as logged-in).
    if ((resp.code == "1011" || resp.code == "9908") && !resp.sessl.empty()) {
        BSphpSeSsL = resp.sessl;
    }
    return resp;
}

ApiResponse BsPhp::get_end_time() {
    return api_call("vipdate.lg");
}

ApiResponse BsPhp::get_code_enabled(const std::string& type) {
    return api_call("getsetimag.in", {{"type", type}});
}

ApiResponse BsPhp::logout_lg() {
    ApiResponse r = api_call("cancellation.lg");
    BSphpSeSsL.clear();
    if (!bootstrap()) {
        return ApiResponse();
    }
