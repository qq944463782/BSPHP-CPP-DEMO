...
struct ApiResponse {
    std::string code;       // 状态码 | 状態碼 | Status code
...
public:
    ...
    /** login.lg with code + session update (code 1011/9908). */
    ApiResponse login_lg(const std::string& user, const std::string& password,
                          const std::string& coode, const std::string& key,
                          const std::string& maxoror);

    /** vipdate.lg - get end time (requires login). */
    ApiResponse get_end_time();

    /** getsetimag.in - captcha enabled switch (e.g. INGES_LOGIN). */
    ApiResponse get_code_enabled(const std::string& type);

    /** cancellation.lg then re-bootstrap session. */
    ApiResponse logout_lg();
    ...
    std::string BSphpSeSsL; // session token，获取后需手动设置供后续 API 使用
